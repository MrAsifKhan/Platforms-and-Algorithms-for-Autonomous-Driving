...
*/
double sigma_init [3] = {0.04, 0.04, 0.01};  //[x,y,theta] initialization noise. Try values between [0.5 and 0.01]
double sigma_pos [3]  = {0.05, 0.05, 0.02}; //[x,y,theta] movement noise. Try values between [0.5 and 0.01]
...
void showPCstatus(vector<Particle> particles){
    ...
    for (int i = 0; i < particles.size(); ++i) {
        ...
        transform.translation() << particles[i].x, particles[i].y, 0;
        renderer.updatePose(circleID+std::to_string(i),transform);
    }

    transform.translation() << best_particles.back().x, best_particles.back().y, 0;
    renderer.updatePose(circleID+std::to_string(NPARTICLES+1),transform);

}

// This function adds the observations from the LiDAR (reflectors) and updates its position
void updateViewerReflector(vector<LandmarkObs> &noisy_observations, pcl::PointCloud<pcl::PointXYZI> reflectorCenter){
...
void PointCloudCb(const sensor_msgs::PointCloud2ConstPtr& cloud_msg){
    ...
    updateViewerReflector(noisy_observations,reflectorCenter);
...
int main(int argc,char **argv)
{
    ...
    remove("./res.txt");
    // This function locates the reflectors within the map
    ...
    for(int i=0;i<NPARTICLES;i++)
        renderer.addCircle(0, 0, circleID+std::to_string(i), 0.1,0,1,0);

    renderer.addCircle(0, 0, circleID+std::to_string(NPARTICLES+1), 0.4,0,1,0); //render the best particle slightly bigger
    renderer.SpinViewerOnce();
